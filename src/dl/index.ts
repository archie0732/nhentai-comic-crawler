...
export const dl = async (album: number | string | Array<string>, dlPath: string = "./") => {
  ...
  else throw new Error("album type is error, only string, number, array can do it");
  ...
  for (let i = 0; i < albums.length; i++) {
    ...
    if (isNaN(Number(albums[i]))) throw new Error("you doujin id have invild character");

    try {
      const doujin = await doujinAPI(albums[i]);

      if (!checkDl(doujin.title.pretty, doujin.id.toString(), map)) {
        console.error(`${i + 1}. ❌ ${chalk.yellow(doujin.id)} ${doujin.title.pretty}` + chalk.red(" - 重複下載"));
        errFlag++;
      } else {
